...
{
    int numBucket = hashTable -> hash(key);
    HashTableEntry *current = hashTable -> buckets[numBucket];
    while (current != NULL) {
        if (current -> key == key) {
            return current;
        }
...
void destroyHashTable(HashTable *hashTable)
{
    for (int i = 0; i < (hashTable -> num_buckets); i++) {
        if (hashTable -> buckets[i]) {
            ...
            HashTableEntry* nextNode = current -> next;
            while (nextNode != NULL) {
                free(current -> value);
                free(current);
                current = nextNode;
...
void *insertItem(HashTable *hashTable, unsigned int key, void *value)
{
    ...
    if (node != NULL) {
        ...
        newNode -> next = hashTable -> buckets[bucketNum];
        hashTable -> buckets[bucketNum] = newNode;
        return NULL;
    }
}
...
void *removeItem(HashTable *hashTable, unsigned int key)
{
    void* ret;
    int bucketNum = hashTable -> hash(key);
    HashTableEntry *node = hashTable -> buckets[bucketNum];
    if ((node) && node -> key == key) {
        ret = node -> value;
        hashTable -> buckets[bucketNum] = node -> next;
        free(node);
        return ret;
    }
    while ((node) && node -> next) {
        if (((node -> next) -> key) == key) {
            HashTableEntry* removedItem = node -> next;
            ret = node -> next -> value;
            node -> next = removedItem -> next;
            free(removedItem);
            return ret;
        } else {
            node = node -> next;
...
void deleteItem(HashTable *hashTable, unsigned int key)
{
    ...
    if ((node) && (node -> key == key)) {
        ...
        while ((node) && node -> next) {
            if (node -> next -> key == key) {
                HashTableEntry* deletedNode = node -> next;
